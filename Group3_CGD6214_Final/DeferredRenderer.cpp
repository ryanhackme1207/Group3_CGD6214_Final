...
bool DeferredRenderer::InitializeShaders(){
    geometryShader=std::make_unique<Shader>(); lightingShader=std::make_unique<Shader>(); debugShader=std::make_unique<Shader>(); hdrShader=std::make_unique<Shader>(); bloomExtractShader=std::make_unique<Shader>(); bloomBlurShader=std::make_unique<Shader>();
    ...
    if(hdrOk) {
        std::cout << "[DeferredRenderer] HDR shader loaded (tone mapping). Program ID=" << hdrShader->GetProgramID() << std::endl;
    } else {
        std::cerr << "[DeferredRenderer] Failed to load HDR shader." << std::endl;
    }
    ...
    bool ok = geoOk && lightOk && hdrOk && bloomExtractOk && bloomBlurOk;
