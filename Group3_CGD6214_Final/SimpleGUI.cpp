...
void SimpleGUI::buildHelpPanel() {
    float startX = m_panelWidth + 30.f + m_panelWidth + 40.f;
    float y = 200.f;
    float w = m_panelWidth + 40.f;
    float h = 300.f;
    addRect(startX, y, w + 10.f, h, 0xAA101018);
    addText(startX + 15, y + 6, "Help / Shortcuts", 0xFFFFFFAA);
    y += 24.f;
    auto line = [&](const char* t) { addText(startX + 15, y, t, 0xFFE0E0E0); y += 14.f; };
    line("Up/Down: navigate hierarchy");
    line("Enter: select/toggle node");
    line("Z/X: expand/collapse node");
    line("F1: toggle GUI");
    line("F6: deferred (enables HDR/Bloom)");
    line("M: cycle MSAA");
    line("B: bloom toggle (F6)");
    line("N: bloom intensity (F6)");
    line("J/K: HDR exposure (F6)");
    line("L: toggle light mode");
    line("1: wireframe ALL buildings");
    line("2: wireframe roofs only");
    line("3: wireframe WHOLE WORLD");
    line("Esc: quit");
    line("NOTE: Bloom & HDR work only in Deferred (F6) mode");
}
...
void SimpleGUI::buildPerformancePanel(float dt) {
    ...
    char line[128];
    std::snprintf(line, 128, "Avg: %.2f ms (%.0f FPS)",
        m_avgFrameMs, (m_avgFrameMs > 0 ? 1000.0f / m_avgFrameMs : 0.f));
    ...
    float graphX = startX + 15.f, graphY = 66.f, graphW = panelW - 30.f, graphH = 90.f;

    // Graph background
    addRect(graphX, graphY, graphW, graphH, 0xFF303045);

    int count = m_historyFilled ? FRAME_HISTORY : m_frameIndex;
    if (count > 1) {
        float maxV = m_maxFrameMs;
        if (maxV < 16.f) maxV = 16.f;

        float barW = graphW / (float)FRAME_HISTORY;
        for (int i = 0; i < count; ++i) {
            int idx = (m_historyFilled ? (m_frameIndex + i) % FRAME_HISTORY : i);
            float v = m_frameTimes[idx];
            float norm = v / maxV;
            if (norm > 1) norm = 1;

            float h = (graphH - 4.f) * norm;
            if (h < 1.f) h = 1.f; // Ensure minimum visibility

            float bx = graphX + 2.f + i * barW;
            float by = graphY + graphH - 2.f - h;

            // Proper ARGB colors
            uint32_t col = (v > 33.0f) ? 0xFFDD4444 : ((v > 22.f) ? 0xFFDDAA44 : 0xFF44DD66);
            addRect(bx, by, barW - 1.5f, h, col);
        }
        ...
        addRect(graphX, ref60Y, graphW, 1.f, 0x88FFFFFF);
        addRect(graphX, ref30Y, graphW, 1.f, 0x88FFAA66);

        addText(graphX + graphW - 50.f, ref60Y - 8.f, "60fps", 0xCCFFFFFF);
        addText(graphX + graphW - 50.f, ref30Y - 8.f, "30fps", 0xCCFFDDAA);
    }
}
