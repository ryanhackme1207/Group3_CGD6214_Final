...
// Helper: convert assumed AARRGGBB to memory order suitable for glVertexAttribPointer reading bytes as RGBA on little-endian.
// Input layout bytes (MSB->LSB): AA RR GG BB. We need bytes (LSB->MSB) = R G B A, so store value as AA BB GG RR.
static inline uint32_t toGPUPacked(uint32_t c){
    uint32_t a = c & 0xFF000000u;          // AA____
    uint32_t r = (c & 0x00FF0000u) >> 16;  // __RR__
    uint32_t g = (c & 0x0000FF00u) >> 8;   // ____GG
    uint32_t b = (c & 0x000000FFu);        // ______BB
    return a | (b << 16) | (g << 8) | r;   // AA BB GG RR (little-endian -> bytes: RR GG BB AA)
}
...
void SimpleGUI::buildShaders() {
    ...
    // Fragment shader: if vUV.x < 0 use solid vertex color (rects). Otherwise sample font mask (red channel) for text alpha.
    const char* fs = "#version 330 core\nin vec2 vUV;in vec4 vCol;out vec4 FragColor;uniform sampler2D uTex;void main(){ if(vUV.x < 0.0){ FragColor = vCol; } else { float mask = texture(uTex,vUV).r; if(mask <= 0.0) discard; FragColor = vec4(vCol.rgb, vCol.a * mask); } }";
...
void SimpleGUI::buildFont() {
    ...
    for (int c = 0; c < 128; ++c) {
        for (int row = 0; row < 8; ++row) {
            unsigned char bits = SIMPLEGUI_FONT[c][row];
            for (int col = 0; col < 8; ++col) {
                if (bits & (1 << col)) { int x = c * 8 + col; int y = row; pixels[y * texW + x] = 255; }
            }
        }
    }
...
void SimpleGUI::BeginFrame() {
    ...
    int mb = glfwGetMouseButton(m_window, GLFW_MOUSE_BUTTON_LEFT);
    m_mousePressed = (mb == GLFW_PRESS && !m_mouseDown);
    m_mouseDown    = (mb == GLFW_PRESS);
    m_vertices.clear();
    ...
    int upState   = glfwGetKey(m_window, GLFW_KEY_UP);
    int enterState = glfwGetKey(m_window, GLFW_KEY_ENTER);
    if (enterState == GLFW_KEY_UNKNOWN) enterState = glfwGetKey(m_window, GLFW_KEY_KP_ENTER);
    m_downPressedFrame  = (downState  == GLFW_PRESS && !m_prevDown);
    m_upPressedFrame    = (upState    == GLFW_PRESS && !m_prevUp);
    m_enterPressedFrame = (enterState == GLFW_PRESS && !m_prevEnter);
    m_prevDown  = (downState  == GLFW_PRESS);
    m_prevUp    = (upState    == GLFW_PRESS);
    m_prevEnter = (enterState == GLFW_PRESS);
    m_hoveredNode = nullptr;

    // Separate debounce per channel so rapid adjustments of different keys work.
    bool shiftHeld = (glfwGetKey(m_window, GLFW_KEY_LEFT_SHIFT)==GLFW_PRESS || glfwGetKey(m_window, GLFW_KEY_RIGHT_SHIFT)==GLFW_PRESS);
    auto adjust=[&](int key,float &v,double &last){ if(glfwGetKey(m_window,key)==GLFW_PRESS){ double now=glfwGetTime(); double interval = 0.07; if(now-last>interval){ float delta = shiftHeld? -0.05f: 0.05f; v = std::max(0.f,std::min(1.f,v+delta)); last=now; } }};
    adjust(GLFW_KEY_T, m_perfColR, m_lastAdjustR);
    adjust(GLFW_KEY_Y, m_perfColG, m_lastAdjustG);
    adjust(GLFW_KEY_U, m_perfColB, m_lastAdjustB);
}
...
void SimpleGUI::drawNodeRecursive(SceneNode* node, int depth, float& y, float panelX, float panelW) {
    if (!node) return;
    float rowH = 16.f; float rowY = y; float labelX = panelX + 10.f + depth * 14.f;
    bool hasChildren = !node->GetChildren().empty();
    bool open = m_openState[node];
    bool hover = regionHit(panelX + 8.f, rowY, panelW - 16.f, rowH); if (hover) m_hoveredNode = node;
    bool navHighlight = (m_navIndex >= 0 && m_navIndex < (int)m_visibleNodes.size() && m_visibleNodes[m_navIndex] == node);
    uint32_t baseCol = 0xFF2A2A60; if (navHighlight) baseCol = 0xFF3A3A80; if (m_selectedNode == node) baseCol = 0xFF5560AA; if (hover && node != m_selectedNode) baseCol = 0xFF444488;
    addRect(panelX + 8.f, rowY, panelW - 16.f, rowH, baseCol);
    if (hasChildren) { uint32_t ic = open ? 0xFF55CC55 : 0xFFCC5555; addRect(panelX + 12.f + depth * 14.f, rowY + 4.f, 8.f, 8.f, ic); }
    addText(labelX + (hasChildren ? 14.f : 0.f), rowY + 4.f, node->name.empty() ? "(unnamed)" : node->name.c_str(), 0xFFE0E0E0);
    if (hover && m_mousePressed) { m_selectedNode = node; m_navIndex = -1; if (hasChildren) { float iconX = panelX + 12.f + depth * 14.f; if (regionHit(iconX, rowY + 4.f, 8.f, 8.f)) m_openState[node] = !open; } }
    if ((navHighlight || m_selectedNode == node) && hasChildren) { if (m_zPressedFrame) m_openState[node] = true; if (m_xPressedFrame) m_openState[node] = false; }
    ...
    y += rowH + 2.f; if (m_openState[node]) for (auto& c : node->GetChildren()) drawNodeRecursive(c.get(), depth + 1, y, panelX, panelW);
}

void SimpleGUI::buildHierarchyPanel() {
    if (!m_sceneRoot) return;
    if (!m_autoExpandedOnce) { openAll(m_sceneRoot); m_autoExpandedOnce = true; }
    if (!m_selectedNode) m_selectedNode = m_sceneRoot;
    m_visibleNodes.clear(); CollectVisible(m_sceneRoot, m_openState, m_visibleNodes);
    if (m_navIndex < 0 || m_navIndex >= (int)m_visibleNodes.size()) { for (size_t i=0;i<m_visibleNodes.size();++i) if (m_visibleNodes[i]==m_selectedNode){ m_navIndex=(int)i; break; }}
    if (m_downPressedFrame && !m_visibleNodes.empty()) m_navIndex = std::min((int)m_visibleNodes.size()-1, m_navIndex+1);
    if (m_upPressedFrame   && !m_visibleNodes.empty()) m_navIndex = std::max(0, m_navIndex-1);
    if (m_enterPressedFrame && m_navIndex>=0 && m_navIndex<(int)m_visibleNodes.size()) { SceneNode* t = m_visibleNodes[m_navIndex]; if (t) { if (m_selectedNode==t) { if(!t->GetChildren().empty()) m_openState[t]=!m_openState[t]; } else m_selectedNode=t; } }
    float startX = m_panelWidth + 30.f; float panelW = m_panelWidth; float y = 15.f; float panelH = 360.f;
    addRect(startX, 5, panelW + 10, panelH + 140, 0xAA101018);
    addText(startX + 15, 10, "Scene Hierarchy (Up/Down, Enter, Z/X)", 0xFFFFFFAA);
    y += 24.f; drawNodeRecursive(m_sceneRoot,0,y,startX,panelW);
    if (m_selectedNode) { addRect(startX+8.f,panelH+20,panelW-16.f,50.f,0x55202040); addText(startX+14.f,panelH+24.f,"Selected:",0xFFAAAAFF); addText(startX+14.f+9*8.f,panelH+24.f,m_selectedNode->name.empty()?"(unnamed)":m_selectedNode->name.c_str(),0xFFFFFFFF);} if(m_hoveredNode&&m_hoveredNode!=m_selectedNode){ addText(startX+15.f,panelH+82.f,(std::string)"Hover: "+(m_hoveredNode->name.empty()?"(unnamed)":m_hoveredNode->name),0xFFCCCC99);} }

void SimpleGUI::buildHelpPanel(){ float startX=m_panelWidth+30.f+m_panelWidth+40.f; float y=200.f; float w=m_panelWidth+40.f; float h=300.f; addRect(startX,y,w+10.f,h,0xAA101018); addText(startX+15,y+6,"Help / Shortcuts",0xFFFFFFAA); y+=24.f; auto line=[&](const char*t){ addText(startX+15,y,t,0xFFE0E0E0); y+=14.f; }; line("Up/Down: navigate hierarchy"); line("Enter: select/toggle node"); line("Z/X: expand/collapse node"); line("T/Y/U: perf color (Shift -=)"); line("F1: toggle GUI"); line("F6: deferred (HDR/Bloom)"); line("M: cycle MSAA"); line("B: bloom toggle"); line("N: bloom intensity"); line("J/K: HDR exposure"); line("L: toggle light mode"); line("1/2/3: wireframe modes"); line("Esc: quit"); line("NOTE: Bloom & HDR only in Deferred mode"); }
...
void SimpleGUI::flush(){ if(m_vertices.empty()) return; GLint vp[4]; glGetIntegerv(GL_VIEWPORT,vp); float L=(float)vp[0]; float T=(float)vp[1]; float W=(float)vp[2]; float H=(float)vp[3]; float R=L+W; float B=T+H; float proj[16]={ 2.f/(R-L),0,0,0, 0,2.f/(T-B),0,0, 0,0,-1,0, (R+L)/(L-R),(T+B)/(B-T),0,1 }; GLboolean depthEnabled=glIsEnabled(GL_DEPTH_TEST); GLboolean cullEnabled=glIsEnabled(GL_CULL_FACE); GLboolean blendEnabled=glIsEnabled(GL_BLEND); glEnable(GL_BLEND); glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA); glDisable(GL_CULL_FACE); glDisable(GL_DEPTH_TEST); glUseProgram(m_shader); glUniform1i(m_uTex,0); glUniformMatrix4fv(m_uProj,1,GL_FALSE,proj); glActiveTexture(GL_TEXTURE0); glBindTexture(GL_TEXTURE_2D,m_fontTex); glBindVertexArray(m_VAO); glBindBuffer(GL_ARRAY_BUFFER,m_VBO); glBufferSubData(GL_ARRAY_BUFFER,0,(GLsizeiptr)(m_vertices.size()*sizeof(Vertex)),m_vertices.data()); glDrawArrays(GL_TRIANGLES,0,(GLsizei)m_vertices.size()); glBindVertexArray(0); if(!blendEnabled) glDisable(GL_BLEND); if(cullEnabled) glEnable(GL_CULL_FACE); else glDisable(GL_CULL_FACE); if(depthEnabled) glEnable(GL_DEPTH_TEST); else glDisable(GL_DEPTH_TEST); }
...
void SimpleGUI::addQuad(float x,float y,float w,float h,uint32_t rgba,float u0,float v0,float u1,float v1){ uint32_t packed=toGPUPacked(rgba); Vertex v; v.color=packed; v.x=x; v.y=y; v.u=u0; v.v=v0; m_vertices.push_back(v); v.x=x+w; v.y=y; v.u=u1; v.v=v0; m_vertices.push_back(v); v.x=x+w; v.y=y+h; v.u=u1; v.v=v1; m_vertices.push_back(v); v.x=x; v.y=y; v.u=u0; v.v=v0; m_vertices.push_back(v); v.x=x+w; v.y=y+h; v.u=u1; v.v=v1; m_vertices.push_back(v); v.x=x; v.y=y+h; v.u=u0; v.v=v1; m_vertices.push_back(v); }
void SimpleGUI::addRect(float x,float y,float w,float h,uint32_t rgba){ // sentinel u=-1 so fragment shader skips sampling
    addQuad(x,y,w,h,rgba,-1.f,0.f,-1.f,0.f);
}
...
void SimpleGUI::buildPerformancePanel(float dt) {
    recordFrameTime(dt);
    float startX = m_panelWidth + 30.f + m_panelWidth + 40.f;
    float panelW = m_panelWidth + 40.f;
    float panelH = 200.f;
    addRect(startX, 5, panelW + 10, panelH + 160, 0xAA101018);
    addText(startX + 15, 10, "Performance", 0xFFFFFFAA);
    char line[128];
    std::snprintf(line,128,"Avg: %.2f ms (%.0f FPS)", m_avgFrameMs,(m_avgFrameMs>0?1000.0f/m_avgFrameMs:0.f)); addText(startX+15,30,line,0xFFE0E0E0);
    std::snprintf(line,128,"Min: %.2f ms  Max: %.2f ms", m_minFrameMs,m_maxFrameMs); addText(startX+15,46,line,0xFFB0B0B0);
    float graphX=startX+15.f, graphY=66.f, graphW=panelW-30.f, graphH=90.f; addRect(graphX,graphY,graphW,graphH,0xFF303045);
    int count = m_historyFilled ? FRAME_HISTORY : m_frameIndex;
    if(count>0){ float maxV=m_maxFrameMs; if(maxV<16.f) maxV=16.f; float barW=graphW/(float)FRAME_HISTORY; uint8_t rBase=(uint8_t)(m_perfColR*255.f); uint8_t gBase=(uint8_t)(m_perfColG*255.f); uint8_t bBase=(uint8_t)(m_perfColB*255.f); for(int i=0;i<count;++i){ int idx=(m_historyFilled?(m_frameIndex+i)%FRAME_HISTORY:i); float v=m_frameTimes[idx]; float norm=v/maxV; if(norm>1) norm=1; float h=(graphH-4.f)*norm; if(h<3.f) h=3.f; float bx=graphX+2.f+i*barW; float by=graphY+graphH-2.f-h; float intensity=0.5f+0.5f*norm; uint8_t r=(uint8_t)std::min(255.f,rBase*intensity); uint8_t g=(uint8_t)std::min(255.f,gBase*intensity); uint8_t b=(uint8_t)std::min(255.f,bBase*(0.75f+0.25f*intensity)); uint32_t col=0xFF000000u | (r<<16)|(g<<8)|b; addRect(bx,by,barW-1.2f,h,col);} float ref60Y=graphY+graphH-2.f-(16.6f/maxV)*(graphH-4.f); float ref30Y=graphY+graphH-2.f-(33.3f/maxV)*(graphH-4.f); addRect(graphX,ref60Y,graphW,1.f,0x88FFFFFF); addRect(graphX,ref30Y,graphW,1.f,0x88FFAA66); addText(graphX+graphW-50.f,ref60Y-8.f,"60fps",0xCCFFFFFF); addText(graphX+graphW-50.f,ref30Y-8.f,"30fps",0xCCFFDDAA); }
    float previewY=graphY+graphH+10.f; uint32_t previewCol=0xFF000000u | ((uint32_t)(m_perfColR*255.f)<<16) | ((uint32_t)(m_perfColG*255.f)<<8) | (uint32_t)(m_perfColB*255.f); addRect(graphX,previewY,graphW,14.f,previewCol); addText(graphX,previewY+18.f,"T/Y/U adjust R/G/B (Shift = -)",0xFFE0E0E0); char rgbLine[64]; std::snprintf(rgbLine,64,"R%.0f G%.0f B%.0f", m_perfColR*255.f,m_perfColG*255.f,m_perfColB*255.f); addText(graphX+graphW-(float)std::strlen(rgbLine)*8.f,previewY+18.f,rgbLine,0xFFCCCCCC); }

// Picking / dragging implementation (restored)
...
static bool sgIntersectAABB(const glm::vec3& ro,const glm::vec3& rd,const glm::vec3& center,const glm::vec3& half,float& t){ glm::vec3 minB=center-half; glm::vec3 maxB=center+half; float tmin=0.f, tmax=1e9f; for(int i=0;i<3;++i){ if(fabs(rd[i])<1e-6f){ if(ro[i]<minB[i]||ro[i]>maxB[i]) return false; } else { float inv=1.f/rd[i]; float t1=(minB[i]-ro[i])*inv; float t2=(maxB[i]-ro[i])*inv; if(t1>t2) std::swap(t1,t2); tmin = t1>tmin? t1 : tmin; tmax = t2<tmax? t2 : tmax; if(tmin>tmax) return false; } } t=tmin; return true; }
